const CACHE_NAME = "cet-reader-cache-v2026-06-11";
const CACHE_PREFIX = "cet-reader-cache";
const AUDIO_CACHE_NAME = "cet-reader-audio-cache-v1";
      ...
      .then((keys) => Promise.all(
        keys
          .filter((key) => (
            ...
            || (key.startsWith(AUDIO_CACHE_PREFIX) && key !== AUDIO_CACHE_NAME)
          ))
...
self.addEventListener("fetch", (event) => {
  ...
  const isStatic = ["script", "style", "manifest", "image"].includes(request.destination)
    || /\.(?:js|css|json|svg|png)$/i.test(url.pathname);
...
function isAudioRequest(url) {
  return url.pathname.includes("/audio/") && /\.mp3$/i.test(url.pathname);
}
